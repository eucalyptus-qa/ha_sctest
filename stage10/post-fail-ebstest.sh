...
attach_volume() {
  ...
  local out=`$TIMEOUT 30 $TOOLSET-attach-volume $vol -i $inst -d /dev/sdb`
  if (( $? != 0 )); then
    return 1
  fi
...
try_dev_stat() {
  for (( i=0; i < 5; i++ )); do
    ...
    $TIMEOUT 30 ssh -i $instancekey root@$instanceip stat -f /dev/sdb
    if (( $? == 0 )); then
      return 0
    fi
  done
...
## Get a test volume
info "Looking for a volume..."
volume=
output=`$TIMEOUT 30 $TOOLSET-decribe-volumes`
if [[ "$output" =~ ^VOLUME.* ]]; then
  volume=`echo "$output" | awk '{print $2}'`
fi
## Make sure the volume is ready
for (( try=0; try < TRIES; try++ )); do
  check_volume_status $volume "in-use"
  ret=$?
  if (( ret == 0 )); then
    break
  fi
...
if (( ret != 0 )); then
  failure "Volume not ready after $TRIES tries"
fi
...
$TIMEOUT 30 ssh -i $instancekey root@$instanceip mkfs.ext2 -q -F /dev/sdb
if (( $? != 0 )); then
  failure "to create file system on attached volume"
fi
info "Mounting the attached volume $volume ..."
$TIMEOUT 30 ssh -i $instancekey root@$instanceip mount /dev/sdb /mnt
if (( $? != 0 )); then
